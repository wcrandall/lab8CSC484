...
// https://github.com/williamfiset/Algorithms/blob/master/com/williamfiset/algorithms/graphtheory/TspDynamicProgrammingRecursive.java
    ...
    public class TspDynamicProgrammingRecursive{
        ...
        // holds the length of distance
        private final int N;
        private final int START_NODE;
        private final int FINISHED_STATE;

        //holds the distances that objects are away from each other
        private double[][] distance;

        // the cost of shortest path
        private double minTourCost = Double.POSITIVE_INFINITY;

        // a list to hold which order the cities are visited for the shortest tour
        private List<Integer> tour = new ArrayList<>();

        // tells if the solver has been ran or not
        //if it's already been ran variables can be referenced without running again
        private boolean ranSolver = false;

        // if the start is not given it will default to starting at 0
        public TspDynamicProgrammingRecursive(double[][] distance) {
            this(0, distance);
        }

        // setsup the class and ready's it to run the tsp function
